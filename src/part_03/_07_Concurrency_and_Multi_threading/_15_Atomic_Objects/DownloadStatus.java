...
public class DownloadStatus {
    ...
    // - It ensures that each thread can safely increment totalBytes even if multiple threads run concurrently.
    ...
    //     3. Before Thread 1 writes 6, Thread 2 reads `totalBytes` as 5 and also wants to increment it to 6.
    //     4. Thread 1 attempts to "swap" 5 with 6 (CAS). Since the current value is still 5, it succeeds.
    //     5. Now `totalBytes` is 6.
    //     6. Thread 2 tries to "swap" 5 with 6, but finds `totalBytes` is now 6, so it retries.
    ...
    public int getTotalBytes() {
        return totalBytes.get(); // Returns current totalBytes value safely
    }

    // 1.3 Increment Method
    // - Increments totalBytes by 1 in a thread-safe manner.
    // - Uses incrementAndGet(), an atomic operation that ensures no interference between threads.
