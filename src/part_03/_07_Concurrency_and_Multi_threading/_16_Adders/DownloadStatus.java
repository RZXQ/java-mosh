...
public class DownloadStatus {
    ...
    // - Compared to AtomicInteger or AtomicLong, which update a single memory location (causing contention),
    //   LongAdder distributes updates across multiple cells.
    // - Each cell holds part of the total, allowing threads to update separate cells concurrently.
    ...
    // - LongAdder.intValue() aggregates (sums) the values in all cells for the final result.
    public int getTotalBytes() {
        return totalBytes.intValue(); // Converts long sum to int
    }

    // 3. incrementTotalBytes() Method - Increments totalBytes by 1.
    // - Adds 1 to one of LongAdder's internal cells, ensuring fast and thread-safe increments.
